...
// A better way to calculate size for P2WPKH (Native SegWit)
fn estimate_vsize(num_inputs: usize, num_outputs: usize) -> u64 {
    // Basic P2WPKH formula: Overhead (10.5) + Inputs (n * 68) + Outputs (n * 31)
    // We round up to be safe.
    let overhead = 11;
    let inputs_size = num_inputs as u64 * 68;
    let outputs_size = num_outputs as u64 * 31;
    overhead + inputs_size + outputs_size
}

#[ic_cdk::update]
pub async fn get_refined_bitcoin_fees(num_inputs: usize, num_outputs: usize) -> Vec<u64> {
    ...
    // Default to a safe minimum (e.g., 2000 msat/B = 2 sat/vB) if percentiles are empty
    let base_rate = if percentiles.is_empty() { 2000 } else { percentiles[50] };
    
    // Convert Millisatoshis/B to Satoshis/B
    let slow_sat_vb = *percentiles.get(10).unwrap_or(&percentiles[0]) / 1000; // Use 10th percentile for "Slow"
    let avg_sat_vb = *percentiles.get(50).unwrap_or(&base_rate) / 1000;
    let fast_sat_vb = *percentiles.get(90).unwrap_or(&percentiles[percentiles.len()-1]) / 1000;

    let v_size = estimate_vsize(num_inputs, num_outputs);
