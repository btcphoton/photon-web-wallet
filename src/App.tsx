  ...
  const handleSendBitcoin = async () => {
    if (sendMode === 'lightning') {
      ...
      try {
        ...
        if (
          normalized.includes('invoice belongs to the sender node') ||
          normalized.includes('belongs to the sender node')
        ) {
          setSendError(
            [
              'This Lightning RGB invoice was generated by the same node you are trying to pay from.',
              '',
              'How to fix:',
              '1) Open a different wallet/node and generate the invoice there.',
              '2) Come back to this wallet and pay that invoice.',
              '',
              'Note: Same-node Lightning transfer requires a dedicated "internal transfer" feature on the backend (database move).',
            ].join('\n')
          )
        } else {
          setSendError(rawMessage || 'Failed to pay Lightning invoice')
        }
      } finally {
